...
def start_server():
    ...
    server.expect("ready\r\n")
    if strace:
        server.real_pid =  server.pid # int(subprocess.check_output("pgrep -P $(pgrep -P %d)" % (server.pid), shell=True))
    else:
        server.real_pid = server.pid  # int(subprocess.check_output("pgrep -P %d" % (server.pid), shell=True))
...
def start_client(port):
    ...
    try:
        ...
        raise ProxyDieException("Can't connect to your proxy")
    if strace:
        proc.real_pid = proc.pid # int(subprocess.check_output("pgrep -P $(pgrep -P %d)" % (proc.pid), shell=True))
    else:
        proc.real_pid = proc.pid # int(subprocess.check_output("pgrep -P %d" % (proc.pid), shell=True))
    return proc
