...
/**********************************
 * Wrappers for robust I/O routines with warnings
 **********************************/
ssize_t Rio_writen_w(int fd, void *userbuf, size_t n)
{
	ssize_t ret_val;

	if ((ret_val = rio_writen(fd, userbuf, n)) != n)
	{
...
ssize_t Rio_readnb_w(rio_t *fp, void *userbuf, size_t n)
{
    ...
    if ((ret_val = rio_readnb(fp, userbuf, n)) != n)
    {
...
ssize_t Rio_readlineb_w(rio_t *fp, void *userbuf, size_t maxlen)
{
    ...
    if ((ret_val = rio_readlineb(fp, userbuf, maxlen)) < 0)
    {
...
ssize_t Open_clientfd_w(char *hostname, char *port)
{
    ...
    if ((ret_val = open_clientfd(hostname, port)) < 0)
    {
