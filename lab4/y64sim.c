...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val) {
    bool_t zero = (val == 0);
    bool_t sign = ((int)val < 0); // BUG should not be int here, it erase first bits (also sign bit) 
    bool_t ovf = FALSE;
    if (op == A_ADD) {
        if (val - argA != argB)  // ((argA < 0) == (argB < 0)) && ((val < 0) != (argA < 0))
            ovf = TRUE;
    } else if (op == A_SUB) {
        if (val + argA != argB)  // ((argA < 0) != (argB < 0)) && ((val < 0) = (argA < 0))
            ovf = TRUE;
...
stat_t nexti(y64sim_t *sim) {
    itype_t icode;           // from I_HALT = 0, I_NOP, I_RRMOVQ, I_IRMOVQ, I_RMMOVQ, I_MRMOVQ, I_ALU, I_JMP, I_CALL, I_RET, I_PUSHQ, I_POPQ, I_DIRECTIVE }
    alu_t ifun = 0;          // { A_ADD, A_SUB, A_AND, A_XOR, A_NONE }
    byte_t instruction = 0;  // unsigned char
    /* get immediate if needed (8 bytes) */
    long_t pc = sim->pc;
    long_t valA = 0;  // int64_t
    long_t valB = 0;
    long_t valC = 0;
    long_t valE = 0;
    long_t valM = 0;
    ...
    if (!get_byte_val(sim->m, pc, &instruction)) {
        err_print("Invalid PC address:  0x%lx ", sim->pc);
        return STAT_ADR;
    }
    icode = GET_ICODE(instruction);
    ifun = GET_FUN(instruction);
    pc++;  // pc+=1 byte
    /* get registers if needed (1 byte) */
    if (icode == I_RRMOVQ || icode == I_ALU || icode == I_PUSHQ || icode == I_POPQ ||
        icode == I_IRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ) {
        if (!get_byte_val(sim->m, pc, &instruction))  // codefun = m->data[pc];
            return STAT_ADR;
        regA = GET_REGA(instruction);
        regB = GET_REGB(instruction);
        pc++;
    }

    /* get immediate if needed (8 bytes) */
    if (icode == I_IRMOVQ || icode == I_RMMOVQ || icode == I_MRMOVQ || icode == I_JMP || icode == I_CALL) {
        if (!get_long_val(sim->m, pc, &valC))
            return STAT_ADR;
        pc += 8;
    }

    /* execute the instruction*/
    if (!get_byte_val(sim->m, sim->pc, &instruction))
        return STAT_ADR;
    cond_t cond = GET_FUN(instruction);
    ...
    switch (icode) {
        ...
        case I_POPQ:
            ...
            break;
        default:
            err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, instruction);
            return STAT_INS;
    }
